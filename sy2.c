...
int main(int argc, char *argv[])
{
    ...
    int stages=3;
    unsigned totsecs=STAGELEN*stages;

    wh_t *ohdr=hdr4chunk(44100, 2, totsecs);
    ...
    int countpstage=STAGELEN*ohdr->sampfq;
    float tf;

    for(i=0;i<stages;++i)
        for (k = 0 ; k < countpstage ; k++) {
            tf = .5*AMPL - (AMPL/M_PI) * sin (TWOPIL*k);
            for(j=1;j<i+1;++j)
                tf += .5*AMPL - (AMPL/M_PI) * ((1./(j+1))*sin ((j+1)*TWOPIL*k));
            bf[i*countpstage*ohdr->nchans +2*k] = tf;

            tf = .5*AMPL - (AMPL/M_PI) * sin (TWOPIR*k);
            for(j=1;j<i+1;++j) 
                tf += .5*AMPL - (AMPL/M_PI) * ((1./(j+1))*sin ((j+1)*TWOPIR*k));
            bf[i*countpstage*ohdr->nchans +2*k+1] = tf;
        }
